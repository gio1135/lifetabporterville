<script lang="ts">
  ...
  async function startTransitionSequence(path: string) {
    ...
    // Allow DOM to update and render the component
    await new Promise(r => setTimeout(r, 10));

    if (transitionContainer) {
      // 1. Fade to black
      await gsap.fromTo(transitionContainer, 
        { opacity: 0 }, 
...
<div 
  ...
  class="fixed inset-0 z-100 bg-black flex items-center justify-center opacity-0"
